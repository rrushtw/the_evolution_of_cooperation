...
### 🛡️ 噪音魯棒策略 (Noise-Robust Strategies)

普通 `TitForTat` 在這個雙重雜訊環境裡有個致命弱點：只要任一方「手滑」背叛一次，雙方就會陷入 C-D-C-D 無止盡互戕（echo 死亡螺旋）。`GenerousTitForTat` 用「報復時 10% 隨機慷慨」打破螺旋，但那是**機率性、無差別**的原諒。以下兩個策略改用**機制性**手段抗噪：

* **`ContriteTitForTat`（懺悔 TFT）**：用「聲望 (standing)」機制分辨「誰理虧」。每個對手各自維護「我」與「對手」的 good/bad 聲望——合作永遠正當，背叛只有對 bad 聲望者才正當。當我**手滑背叛**了一個 good 聲望的對手，我的聲望會掉成 bad；對手據此「正當報復」時其聲望仍是 good，於是我**不反報復、改合作贖罪**，雙方在 2 回合內回到互助，只損失 1 次報復。面對真正的剝削者則照常報復，不吃虧。
* **`OmegaTitForTat`（Omega TFT）**：在 TFT 之上加兩個 per-opponent 計數器。**死鎖計數**偵測對手最近兩手是否一直「交替」(CD/DC echo)，連續達門檻就**主動合作一手**打破死鎖；**隨機計數**累積對手的「亂度」，超過門檻就判定對手不可溝通而**永久背叛**（對純隨機對手 AllD 是最佳解）。採 Axelrod 函式庫的經典參數（死鎖門檻 3 / 隨機門檻 8）。
